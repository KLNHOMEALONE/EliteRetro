...
- [ ] **0.1** Create `GameConstants.cs` — all scale values (planet radius 24576, station distance 65536, bubble radius 57344, slot counts, sun radii multipliers)
- [ ] **0.2** Create `EntityInstance.cs` — runtime entity wrapper (position, velocity, orientation, speed, energy, slot index, destroyed flag)
- [ ] **0.3** Create `OrientationMatrix.cs` — 3×3 rotation matrix (nosev/roofv/sidev), identity constructor, Minsky RotatePosition, ApplyUniverseRotation, Tidy orthonormalization, TransformLocalToWorld
- [ ] **0.4** Create `ShipBlueprint.cs` — static ship data structures (VertexDef, EdgeDef, FaceDef, ShipBlueprint), port all ship characteristics from docs
- [ ] **0.5** Create `ShipInstance.cs` — 36-byte-equivalent runtime data (position, orientation vectors, speed, acceleration, roll/pitch counters, flags, AI, energy)
- [ ] **0.6** Create `PlanetModel.cs` — planet wireframe geometry
- [ ] **0.7** Create `SunModel.cs` — sun wireframe geometry
- [ ] **0.8** Create `FlightController.cs` — keyboard input mapping (arrows=pitch, Q/W=roll), angle scaling (/256), Alpha/Beta properties
- [ ] **0.9** Create `LocalBubbleManager.cs` — slot array[20], spawn/despawn, culling beyond 57344, safe zone trigger, station spawn/sun removal, energy bomb

## Phase 1: Local Bubble & Entity Lifecycle

- [ ] **1.1** Implement slot allocation in LocalBubbleManager — reserved slot 0 (planet), slot 1 (sun/station), slots 2+ (dynamic)
- [ ] **1.2** Implement spawn logic — find first empty slot from index 2, reject if full
- [ ] **1.3** Implement despawn logic — mark destroyed, shuffle remaining down
- [ ] **1.4** Implement bubble culling — distance check > 57,344 per frame
- [ ] **1.5** Implement orbit point calculation — `planetPos + 2 * planetNosev * PlanetRadius`
- [ ] **1.6** Implement safe zone trigger — bounding box check at 192 local coords, spawn station, remove sun
- [ ] **1.7** Implement station orientation — invert nosev to face planet center
...
- [ ] **2.1** Implement `OrientationMatrix.RotatePosition()` — Minsky circle algorithm for entity positions
- [ ] **2.2** Implement `OrientationMatrix.ApplyUniverseRotation()` — Minsky rotation for orientation vectors
- [ ] **2.3** Implement `OrientationMatrix.Tidy()` — normalize nosev, orthogonalize roofv (smallest-component division), cross-product sidev
- [ ] **2.4** Implement `OrientationMatrix.ApplyOwnRotation()` — fixed 1/16 rad rotation for AI ships
- [ ] **2.5** Modify `SpaceScene` — replace `_rotation` Euler angles with Minsky universe rotation
- [ ] **2.6** Add view switching — V key cycles Front/Rear/Left/Right, apply axis transforms
- [ ] **2.7** Implement periodic tidying — round-robin across entities each frame
- [ ] **2.8** Implement forward movement — entity moves along its own nosev by speed

## Phase 3: Galaxy Generation (Tribonacci)

- [ ] **3.1** Create `GalaxySeed` struct — three 16-bit seeds, Twist(), NextGalaxy(), Copy()
- [ ] **3.2** Implement twist algorithm — `s0'=s1, s1'=s2, s2'=s0+s1+s2` with 16-bit wraparound
- [ ] **3.3** Implement next-galaxy — rotate each byte pair left by 1 bit
- [ ] **3.4** Implement system data derivation — economy (s0_hi & 0b111), government ((s1_lo >> 3) & 0b111), tech level, population, productivity, radius, species, galactic coords
- [ ] **3.5** Implement name generation — 2-letter token table (indices 129-159), 3-4 tokens per name, twist between tokens
- [ ] **3.6** Add anarchy/feudal economy constraint — force bit 1 of economy (no Rich for Anarchy/Feudal)
...
- [ ] **3.9** Replace existing `GalaxyGenerator` — use Tribonacci instead of simple RNG
...
**Phase 0: Foundation** — building core data structures and constants.

Next immediate task: **0.1** Create `GameConstants.cs`.
